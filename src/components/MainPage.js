...
export default class MainPage extends Component {
  ...
  handleCollegeChange = (selectedcollege) => {
    ...
    const fetchcollegedetailsurl =
      '/college/college-details/' + selectedcollege.value;
    instance.get(fetchcollegedetailsurl).then((response) => {
    ...
    instance.get(studenturl).then((res) => {
      if (res.data.length > 0) {
        this.setState({
    ...
    instance.get(similarcollegeurl).then((res) => {
      this.setState({
  ...
  handleStudentChange = (selectedstudent) => {
    ...
    instance.get(studentdataurl).then((res) => {
      if (res.data.length > 0) {
        this.setState({
      ...
      <div className='mt-5'>
        <Row>
          <Col md='4'>
            ...
            <div
              className='mb-4'
              style={{ display: this.state.showstudent ? 'block' : 'none' }}
            >
              <p>Select Student: </p>
              <ReactSelect
        ...
        <Row>
          <Col md='8'>
            <div
              className='ml-3 mt-3'
              style={{
                display: this.state.showcollegedetails ? 'block' : 'none',
              }}
            >
              <div>
                ...
                <ul>
                  ...
                  <DataDetailList
                    label='Year Founded'
                    value={this.state.collegeyearfounded}
                  />
                  <DataDetailList
                    label='No. of Students'
                    value={this.state.collegenoofstudents}
                  />
                  <DataDetailList
                    label='Location'
                    value={
                      this.state.collegecity +
                      ', ' +
                      this.state.collegestate +
                      ', ' +
                      this.state.collegecountry
                    }
                  />
                  <DataDetailList
                    label='Courses Provided'
            ...
            <div
              className='ml-3 mt-3'
              style={{
                display: this.state.showstudentdetails ? 'block' : 'none',
              }}
            >
              <div>
                ...
                <ul>
                  ...
                  <DataDetailList
                    label='Batch Year'
                    value={this.state.studentbatchyear}
                  />
                  <DataDetailList
                    label='Skills'
        ...
        <div
          className='mt-4'
          style={{ display: this.state.showsimilarcolleges ? 'block' : 'none' }}
        >
